...
class Bip32Version {
  const Bip32Version({required this.public, required this.private})
    : assert(public >= 0 && public <= 0xFFFFFFFF),
      assert(private >= 0 && private <= 0xFFFFFFFF);
...
abstract final class Networks {
  ...
  static const NetworkType bitcoin = NetworkType(
    wif: 0x80,
    bip32: Bip32Version(public: 0x0488b21e, private: 0x0488ade4),
  );

  /// Bitcoin testnet / regtest: `tpub` / `tprv`, WIF `0xef`.
  static const NetworkType bitcoinTestnet = NetworkType(
    wif: 0xef,
    bip32: Bip32Version(public: 0x043587cf, private: 0x04358394),
  );

  /// Litecoin mainnet (legacy version bytes used in upstream fixtures).
  static const NetworkType litecoin = NetworkType(
    wif: 0xb0,
